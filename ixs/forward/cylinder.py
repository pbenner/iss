...
from .create_form import Simulation
...
class Cylinder(Simulation):
    ...
    def __points_in_cylinder(self, pt1, pt2, r, q):
        """
        Maps the cylinder onto 3D space
        Arguments:
            pt1 ([int, int, int]): coordinate of one of the cylinder bases' center 
            pt2 ([int, int, int]): coordinate of the second of the cylinder bases' center 
            r (float) : cylinder radius
            q (float): cylinder length            
        Returns:
            np array 1D mask for cylinder-pixels
        """
        vec = pt2 - pt1
    ...
    def __cylinder_pbc(self,mask):
        """
        Checks if the current cylinder satisfies the periodic boundary conditions
        Arguments:
            a new cylinder mask
        """
    ...
    def __generate_cylinder(self, radius, height, center, theta, phi):
        """
        Generates a new cylinder and accepts it into the simulated density box, if it satisfes the periodic boundary conditions
        Arguments:
            radius (float): radius of the cylinder
            height (float): height of the cylinder
            center ([float, float, float]): center of the base of one end of the cylinder
            phi (int): a rotation about the cylinder axis
            theta (int): rotation of cylinder from the central axis
        """
    ...
    def save_data(self, uncertainty = "ISigma", path='.', for_SasView = True):
        """
        ...
        if for_SasView:
            data.assign(Qx = data.Qx/10, Qy = data.Qy/10, I = data.I/100, ISigma = data.ISigma/100).to_csv(path+'/polydispersed_cylinders_{r}_{h}.dat'.
            format(r = int(self.rMean*1000), h = int(self.hMean*1000)), header=None, index=None, columns=["Qx", "Qy", "I", uncertainty])
        else:
            data.to_csv(path+'/polydispersed_cylinders_{r}_{h}.dat'.
            format(r = int(self.rMean*1000), h = int(self.hMean*1000)), header=None, index=None, columns=["Qx", "Qy", "I", uncertainty])
