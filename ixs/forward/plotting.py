...
def plot_3D_structure(entity, grid= None, realspace = True, path = None, figsize = 10):
    """ 
    plots the full 3D structure
    Arguments: 
        entity: the 3D entity to plot
        grid: grid points of the entity to plot, used for labeling axes, if None labelling respective to the size of structure
        realspace[boolean]: if True considered to be density, if false, the Fourier Transform, values are log-values
        path: path to save figure 
        figsize(int): the size of the image created
    """
    ...
    if realspace:
        ...
        ax.set_ylabel(r'$Y (nm)$', fontsize=15)
        ax.set_zlabel(r'$Z (nm)$', fontsize=15)
        title = 'The density in 3D'
    else: # reciprocal
        img = ax.scatter3D(xx, yy, zz, zdir='z', c= np.log(entity[xx,yy,zz]),  cmap="Greys" , s = 2, marker = 'o')
        ax.set_xlabel(r'$Q_x nm^{-1}$', fontsize=15)
        ax.set_ylabel(r'$Q_y nm^{-1}$', fontsize=15)
        ax.set_zlabel(r'$Q_z nm^{-1}$', fontsize=15)
        title = 'Fourier transformed density in 3D'
    
    if not realspace:
        cbar = plt.colorbar(img, shrink = 0.8)
        cbar.set_label('log FFT', size=16)# only include colorbar when showing a FT
    if grid is not None:
        ...
        ax.set_xlim(0,entity.shape[0])
        ax.set_ylim(0,entity.shape[1])
        ax.set_zlim(0,entity.shape[2])        
...
def plot_Q_vs_I(binned_data, yerr = 'ISigma', xlim = None, path = None, figsize = 10):
    """
    ...
    binned_data.plot('Q', 'I', yerr = yerr, 
            figsize=(figsize,figsize),logx = True, logy = True, xlabel = 'q', ylabel = 'I', title = 'rebinned Q vs I')
    if xlim:
...
def plot_slices_ft(simulation):
    """
    Plots central slices of the Fourier transform given all dimensions x/y/z.
    Arguments:
        simulation : class instance with calculated attributes
    """
    non_zero_values = torch.where(simulation._FTI_custom ==0, 1e-5, simulation._FTI_custom)
    vmin = myround(np.log(non_zero_values).min())
    vmax = myround(np.log(non_zero_values).max())

    fig,axs = plt.subplots(1,3,figsize = (20,10))
    ax = axs[0]
    extent = [simulation.qx.min(), simulation.qx.max(), simulation.qx.min(), simulation.qx.max()] if 'qx' in dir(simulation) else [0,simulation.density.shape[1],0,simulation.density.shape[2]]
    im = ax.imshow(np.log(simulation._FTI_custom[simulation._FTI_custom.shape[0]//2+1,:,:]), cmap = 'Greys', vmin = vmin, vmax = vmax, extent = extent)
    ax.set_ylabel(r"$Q_z\,nm^{-1}$", fontsize = 16)
    ax.set_xlabel(r"$Q_y\,nm^{-1}$", fontsize = 16)
    ax.tick_params(labelsize = 15)
    ax = axs[1]
    extent = [simulation.qx.min(), simulation.qx.max(), simulation.qx.min(), simulation.qx.max()] if 'qx' in dir(simulation) else [0,simulation.density.shape[0],0,simulation.density.shape[2]]
    im = ax.imshow(np.log(simulation._FTI_custom[:,simulation._FTI_custom.shape[1]//2+1,:]), cmap = 'Greys', vmin = vmin, vmax = vmax, extent = extent)
    ax.set_ylabel(r"$Q_z\,nm^{-1}$", fontsize = 16)
    ax.set_xlabel(r"$Q_x\,nm^{-1}$", fontsize = 16)
    ax.tick_params(labelsize = 15)
    ax = axs[2]
    extent = [simulation.qx.min(), simulation.qx.max(), simulation.qx.min(), simulation.qx.max()] if 'qx' in dir(simulation) else [0,simulation.density.shape[0],0,simulation.density.shape[1]]
    im = ax.imshow(np.log(simulation._FTI_custom[:,:,simulation._FTI_custom.shape[2]//2+1]), cmap = 'Greys', vmin = vmin, vmax = vmax, extent = extent)
